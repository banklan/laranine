            ...
            <v-col cols="12" md="10">
                ...
                <template v-else>
                    <div class="grey_text--text text-center mb-5">
                        Search for <strong><em>{{ $route.query.q }}</em></strong> returned {{ users.length }} users.
                    </div>
                        ...
                        <template v-else>
                            <v-card-text>
                                <v-alert type="error" border="left" class="mt-8 mx-3">
                                    Search for {{ $route.query.q }} returned no user.
                                </v-alert>
                            </v-card-text>
                        </template>
                    </v-card>
...
export default {
    ...
    methods: {
        getResult(){
            this.isLoading = true
            axios.post(this.api + '/auth-admin/search_for_admins', {
